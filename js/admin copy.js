...
async function loadAdminPostList() {
    ...
    if (!posts || posts.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="padding: 20px; text-align: center; color: #94a3b8;">Chưa có bài viết nào trong hệ thống.</td></tr>`;
        return;
    }

    let rowsHTML = '';
    posts.forEach(post => {
        const imgDisplay = post.image_url ? `<img src="${post.image_url}" style="width:60px; height:40px; object-fit:cover; border-radius:4px;">` : 'Không có ảnh';
        rowsHTML += `
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.05); transition: 0.2s;">
                <td style="padding: 15px 20px;">${imgDisplay}</td>
                <td style="padding: 15px 20px; font-weight:600; color:#fff;">${post.title}</td>
                <td style="padding: 15px 20px; color:#94a3b8; font-family:monospace;">${post.slug}</td>
                <td style="padding: 15px 20px; text-align: right; white-space: nowrap;">
                    <button type="button" onclick="prepareEditPost('${post.slug}')" style="background:#1e293b; color:#fff; border:1px solid rgba(255,255,255,0.1); padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:600; font-size:13px; margin-right:5px;">Sửa</button>
                    <button type="button" onclick="deletePostAction('${post.id}')" style="background:#7f1d1d; color:#fff; border:none; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:600; font-size:13px;">Xóa</button>
                </td>
            </tr>
        `;
    });
...
async function processSubmitPost() {
    ...
    if (!title || !slug || !excerpt) {
        showAlert("Thiếu thông tin", "Vui lòng nhập Tiêu đề, Slug và Trích dẫn!", "warning"); return;
    }

    btn.disabled = true;
    try {
        btn.innerText = "ĐANG KIỂM TRA MÃ ID...";
        const { data: existingPost } = await supabaseClient.from('posts').select('id').eq('slug', slug).maybeSingle();
        if (existingPost) {
            showAlert("Trùng ID", `Mã ID "${slug}" đã tồn tại! Vui lòng đổi mã khác.`, "error");
            btn.innerText = "Lưu Và Đăng Bài"; btn.disabled = false; return;
        }
        ...
        const payload = { title, slug, excerpt, content, image_url: imageUrl };
        const { error } = await API.savePost(payload);

        if (error) {
            showAlert("Lỗi hệ thống", error.message, "error");
        } else {
            Swal.fire({ title: "Thành công!", text: "Đăng bài viết mới thành công!", icon: "success", background: '#1e293b', color: '#fff', confirmButtonColor: '#E65100' });
    ...
    btn.innerText = "Lưu Và Đăng Bài"; btn.disabled = false;
}

async function deletePostAction(id) {
    const result = await showConfirm("Xóa bài viết?", "Sếp chắc chắn muốn xóa bài viết này chứ?");
    if (!result.isConfirmed) return;
    ...
    if (success) {
        ...
        showAlert("Lỗi!", "Hệ thống không xóa được bài viết!", "error");
    }
}

async function prepareEditPost(slug) {
    const post = await API.getPostBySlug(slug);
    if (!post) { showAlert("Lỗi!", "Không tìm thấy dữ liệu bài viết!", "error"); return; }
...
async function processUpdatePost() {
    const btn = document.getElementById('btnUpdatePost');
    const id = document.getElementById('editPostId').value;
    const title = document.getElementById('editPostTitle').value.trim();
    const slug = document.getElementById('editPostSlug').value.trim().toLowerCase();
    ...
    if (!title || !slug || !excerpt) {
        showAlert("Thiếu thông tin", "Vui lòng nhập đủ Tiêu đề, Slug và Trích dẫn!", "warning"); return;
    }

    btn.disabled = true; btn.innerText = "ĐANG XỬ LÝ...";

    try {
        const { data: existingPost } = await supabaseClient.from('posts').select('id').eq('slug', slug).neq('id', id).maybeSingle();
        if (existingPost) {
            showAlert("Trùng ID", `Mã ID "${slug}" đã bị trùng với bài khác!`, "error");
            btn.innerText = "LƯU BÀI VIẾT"; btn.disabled = false; return;
        }
        ...
        const payload = { id, title, slug, excerpt, content };
        ...
        if (error) {
            ...
            Swal.fire({ title: "Thành công!", text: "Đã cập nhật bài viết!", icon: "success", background: '#1e293b', color: '#fff', confirmButtonColor: '#E65100' });
    ...
    btn.innerText = "LƯU BÀI VIẾT"; btn.disabled = false;
}
...
// ==========================================
// HỆ THỐNG ĐĂNG NHẬP & KHỞI ĐỘNG
// ==========================================

const ADMIN_PASSWORD = "123"; // SẾP ĐỔI MẬT KHẨU Ở ĐÂY NHÉ!

document.addEventListener('DOMContentLoaded', () => {
    // Kiểm tra xem đã lưu trạng thái đăng nhập trong trình duyệt chưa
...
function initAdminData() {
    ...
    loadAdminOrderList(); 
    loadIntroData(); 
    loadAdminPostList(); // Kích hoạt tải dữ liệu bài viết
}